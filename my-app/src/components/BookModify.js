              ...
              <div>
                <span>
                  ...
                  <input
                    type='time'
                    style={{ width: '130px' }}
                    value={topMg.start_time || ""}
                    onChange={(e) =>
                      setTopMg((prev) => ({ ...prev, start_time: e.target.value }))
                    }
                  /> </span>
                <span>
                  <label>End</label>
                  <input
                    type='time'
                    style={{ width: '130px' }}
                    value={topMg.end_time || ""}
                    onChange={(e) =>
                      setTopMg((prev) => ({ ...prev, end_time: e.target.value }))
                    }
                  />
                </span>
                <span>
                  {(() => {
                    ...
                    const start = toMinutes(topMg.start_time);
                    const end = toMinutes(topMg.end_time);
                    const isValid = start !== null && end !== null && end - start >= 60;

                    return (
                      isValid && (
                        <button className='btnModify'
                          onClick={async () => {
                            try {
                              const res = await fetch(
                                `http://localhost:5000/api/bookinghours/${bookingId}`,
                                {
                                  method: "PUT",
                                  headers: { "Content-Type": "application/json" },
                                  body: JSON.stringify({
                                    // id_bookingdates: bookingId,
                                    date: new Date(topMg.date).toLocaleDateString("fr-CA"),
                                    start_time: topMg.start_time,
                                    end_time: topMg.end_time,
                                  }),
                                }
                              );
