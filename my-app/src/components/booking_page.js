...
function Booking_page() {
  ...
  function appel(callback) {
    ...
    fetch('http://localhost:5000/api/' + room_id + '/roomdispo') // to get room_disposition.id_room
      .then(res => res.json())
      .then((data1) => {
  ...
  function appel1(callback1) {
    fetch('http://localhost:5000/api/' + room_id + '/roomdispo') // to get room_disposition.id_disposition
      .then(res => res.json())
      .then((data1) => {
        // const [anne] = [data1[0].id, data1[0].id_room, data1[0].id_disposition, data1[0].capacity, data1[0].price]
